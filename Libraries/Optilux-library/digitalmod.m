...
%
%   'gauss': Gaussian shaped pulses. The standard deviation [symbols] in 
%       the time domain can be set through the optional parameter PAR.
%
...
%           by MYFILTER.
%           Note: to generate Gaussian-shaped pulses with standard 
%               deviation sigma [symbols] in the time domain, use:
%
%               PTYPE = 'gauss';
...
%
%
%   See also INIGSTATE, DATAPATTERN, MZMODULATOR, IQMODULATOR, LASERSOURCE.
%
%   References:
...
if flag == 1    % filter the signal
    ...
    if strcmpi(ptype,'rootrc') % square-root raised cosine
        Hfir = sqrt(Hfir*Nsps);
        % *Nsps: because I'm using filters normalized in peak spectrum (as if
        % symbol time was 1)
    end
end
% elec = conv(Aku,Hfir,"same"); % louis
...
elseif strcmpi(par.norm,'no')
    ...
    error('Unknwon normalisation method');
end
elec = elec/sqrt(avge);
...
switch ptype
    case {'rc','rootrc'}
%         tfir                        = (1/par.duty)*(-8:1:8)'; % louis, trying to match python
                                                                % rrcfir
