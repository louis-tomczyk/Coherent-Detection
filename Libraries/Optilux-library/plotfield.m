% function [tim,freq] = plotfield(E,flag,o1,o2)
...
%           separate lines. If 'tot', the total power is plotted (default).
%
...
if isstruct(x)
    errmsg=checkfields(x,{'norm','power'}); 
    if ~isempty(errmsg), error('optilux:plotfield',errmsg); end
end
...
if ~isempty(imm) || ~isempty(imd) || ~isempty(idm) || ~isempty(idd)
    col1 = [col(isstrprop(col,'alpha')|col=='+'),'-'];
else
    col1 = [col(isstrprop(col,'alpha')|col=='+'),'--'];
end
...
if ~isempty(x) && isfield(x,'power') 
    if strcmpi(x.power,'xy')
        istot = false;
    elseif strcmpi(x.power,'tot')
        istot = true;
    else
        error('field power can be only xy so far.');
    end
else
...
if strcmpi(flag(3),'p') || strcmpi(flag(4),'a')
    ...
    if strcmpi(flag(3),'p') %%% power, frequency-domain
        ...
        grid on, hold on
        xlabel(freqlab,'fontweight','bold')
        ylabel(psdlab,'fontweight','bold')
    ...
    if strcmpi(flag(4),'a') %%% phase, frequency-domain
        ...
        grid on, hold on
        xlabel(freqlab,'fontweight','bold')
        ylabel('Spectrum phase [rad]','fontweight','bold')
    end
