...
%
%
%   See also MZMODULATOR, DIGITALMOD, DATAPATTERN, INIGSTATE
%
%   References:
%
...
if exist('options','var')
    errmsg=checkfields(options,{'n0','linewidth','pol'});
    if ~isempty(errmsg), error('optilux:lasersource',errmsg);end
    ...
    if isfield(options,'pol') 
        if strcmpi(options.pol,'single')
            Npol = 1; % scalar propagation
        else
            error('Use ''single'' for single polarization.');
        end    
    else
...
if any(linewidth)
    ...
    % louis tomczyk version
%     freq_noise_var  = sqrt(2*pi*linewidth./GSTATE.FSAMPLING);
%     freq_noise      = freq_noise_var*[zeros(1,Nch),randn(Nsamp-1,Nch)];
%     freq_noise      = freq_noise-circshift(freq_noise,1);
%     phase_noise     = cumsum(freq_noise,1);
% 
%     E.field(:,1:Npol:Nch*Npol) = fastexp(phase_noise) .* E.field(:,1:Npol:Nch*Npol);
end
